...
const {title,description,cover,genre,rating,video,platform,launchDate,isCompetitive} = req.body

if(title === "" ||description === "" ||cover === "" ||genre === "" ||rating === "" ||video === "" ||platform === "" ||launchDate === "" ||isCompetitive === ""){
  res.status(400).render("admin/admin-new-game",{errorMessage: "Los campos no pueden estar vacios"})
  return
}


try {
  await Game.create({title,description,cover,genre,rating,video,platform,launchDate,isCompetitive })
  res.redirect("/game/list")
} catch (error) {
  next(error)
